...
async function fetch_tweets(options, cursor = null) {
	try {
		...
		var variable = `?variables=%7B%22userId%22%3A%22${user_id}%22%2C%22count%22%3A${count}%2C${final_cursor}%22includePromotedContent%22%3Atrue%2C%22withCommunity%22%3Atrue%2C%22withVoice%22%3Atrue%7D`;

		var final_url = `${base_url}${variable}&features=${features}`;
...
async function run(options) {
	...
	features = options.headers.features;
	deletedCount = 0;
	tweets_to_delete = [];
